...
OUTPUTCSV = f"/output/radiomic_features_{datetime.now().strftime('%Y-%m-%d')}.csv"
...
def run(case):
    ...
    try:
        ...
        filename = (
            r"features_" + str(case["Reader"]) + "_" + str(case["Patient"]) + ".csv"
        )
        ...
        if os.path.isfile(output_filename):
            ...
            ptLogger.info(
                "Patient %s read by %s already processed...",
                case["Patient"],
                case["Reader"],
            )
            ...
            feature_vector.update(
                extractor.execute(imageFilepath, maskFilepath, label=label)
            )
