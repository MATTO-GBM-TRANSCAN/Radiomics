...
def getHEqualizedImage(inputImage, inputMask, **kwargs):
    """
    Applies histogram matching to the region of interest (ROI) defined by a binary mask in the source image.
    
    Parameters:
        inputImage (sitk.Image): The input MR image (as a SimpleITK image).
        inputMask (sitk.Image): The binary mask image (non-zero values define the ROI).
    
    Returns:
        sitk.Image: The output image where the ROI has been histogram matched and the rest remains unchanged.
    """
    global logger
    # Ensure mask is of type UInt8 and extract ROI from the source image
    mask = sitk.Cast(inputMask, sitk.sitkUInt8)
    roi_source = sitk.Mask(inputImage, mask)
    
    alpha = kwargs.get("alpha", 0.0)  # Contrast Enhancement: 0.0 acts like the classical histogram equalization method
    beta = kwargs.get("beta", 1.0)    # Noise suppression
    logger.info('Computing Histogram Equalized with alpha %f, beta %f', alpha, beta)

    ahe_filter = sitk.AdaptiveHistogramEqualizationImageFilter()
    ahe_filter.SetAlpha(alpha)
    ahe_filter.SetBeta(beta)
    
    # Execute the filter to produce the equalized image
    roi_matched = ahe_filter.Execute(roi_source)

    # Recombine the histogram-matched ROI with the original image using the mask.
    mask_float = sitk.Cast(mask, inputImage.GetPixelIDValue())
    output_image = roi_matched * mask_float + inputImage * (1 - mask_float)
    
    yield output_image, "histogram-equalized", kwargs